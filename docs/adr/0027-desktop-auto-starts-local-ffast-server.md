...
> `ConnectionManager`). The shim-deletion below (no `_refreshLocalSceneAdapter`,
> no `serverConnection is None` guards) is still accurate. Read "Implementation order"
...
is disabled until the connection is established. After this, `env.remote`'s
**Server Connection** is always set on desktop — the same as remote mode — and
the local shim path (`_refreshLocalSceneAdapter`, all `serverConnection is None`
guards) is deleted.
