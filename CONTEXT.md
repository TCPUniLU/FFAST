...
The single module (`ffast/metrics/execution.py`, ADR 0035; seam finished in ADR 0046) that resolves a Metric's inputs, dependencies, and Compute Parameters once into an ordered, transport-ready **Execution Plan** — the sole place optional-input and missing-dependency semantics are defined. `build_execution_plan` walks the Metric Graph once (dependency-ordered steps, each input pre-classified as a raw value or a dependency output, Compute Parameters pre-filtered from presentation ones); `run_plan` is the shared driver that wires dependency outputs, consults the result cache, and constructs every Metric Result. The **MetricExecutor** seam is one method — `run(id, source: InputSource, parameters)` — behind which the in-process executor and the **Metric Worker Pool** are adapters that differ only in transport (direct call vs pickle-to-worker); no caller re-flattens or re-walks (the former `build_metric_inputs` double-walk is gone, ADR 0046). Every caller names its **Input Source**: the panel path passes `_ResolverSource` (`InputResolver` sourcing whole-dataset arrays), atom coloring passes a **frame-scoped source** (current-frame arrays, same `(found, value)`/never-raise contract), and pre-resolved-array callers pass `FlatInputSource`. The executor is **injected at Environment construction** (ADR 0046, superseding ADR 0016's in-process-on-server line): the headless/server Environment injects a `WorkerProcessExecutor` so metric code never runs in the long-lived server process (the Worker Pool Key Constraint), the desktop/client Environment injects an `InProcessExecutor`, and the one server instance serves both server-side metric paths (atom coloring and `REQUEST_METRIC`). The plan is picklable so a worker can consume it without an Environment. Ordering + cycle detection live only in this walk; `MetricGraph` is now purely the startup/CLI **validator** (`freeze`), its dead `compute_plan`/`dependencies_of` removed.
...
A user's client-local cosmetic customization of one **Panel** — x/y axis label text and font size, and legend text, font size, and position (including a per-**Series** entry-text override keyed by that Series' `(dataset, model)` identity). Scoped to a Panel by content-based identity (**Analysis Tab** name, **Panel Kind**, bound **Metric IDs**), so it survives Panel rebuilds and TOML reordering, and never touches computation identity. Persisted to a local, app-managed file, distinct from **Visualization Configuration**: a Visualization Configuration is a curated, mergeable, shareable partial override a person authors; a Panel Display Override is silently rewritten by the app whenever the user edits a label or drags a legend, and is never hand-authored. Also distinct from a **Presentation Parameter**: a Presentation Parameter is a Metric-declared, schema-validated display setting (e.g. colormap, units) that generates a control through the normal Parameter Schema pipeline, while a Panel Display Override is pure Panel-local UI chrome with no Metric or schema involved. Shares its persistence file with **Colorbar Display Override** (a second top-level key in the same app-managed file), but the two are kept as separate terms rather than one umbrella concept (see [ADR 0029](docs/adr/0029-display-overrides-client-local-cosmetic-state.md)).
_Avoid_: plot style, panel preferences, panel settings

### Colorbar Display Override
A user's client-local cosmetic customization of the **Loupe** atom-coloring colorbar — label text, unit text, font size, and free-floating position — keyed by the client-tracked **Metric ID** currently driving atom-coloring (the coloring-source combo's label→Metric ID map), not by anything in the wire-level `AtomColorBy` descriptor, which carries no identity by design (ADR 0016). Implemented as a Qt-native overlay widget over the vispy canvas (replacing the vispy `ColorBarWidget`'s grid-cell placement, which cannot float or accept drag/edit events), the same technique already used for the atom-select toolbar. Persisted alongside **Panel Display Override** in the same app-managed file. Excludes the colorbar's `vmin`/`vmax` range, which is a **Presentation Parameter** concern (schema-driven, server-validated), not cosmetic UI chrome. See [ADR 0029](docs/adr/0029-display-overrides-client-local-cosmetic-state.md).
_Avoid_: colorbar style, colorbar settings, legend (3D has no plot legend, only the value-coloring colorbar)
...
- A **Colorbar Display Override** is the 3D-view counterpart to a **Panel Display Override** — same client-local, content-keyed, app-managed-file pattern, applied to the **Loupe** atom-coloring colorbar instead of a Panel; the two share one persistence file but are named as separate terms
